...
class DocumentController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				document.id LIKE ? OR 
				document.tittle LIKE ? OR 
				document.content LIKE ? OR 
				document.date LIKE ?
			)";
			$search_params = array(
				"%$text%","%$text%","%$text%","%$text%"
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("document.id", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("document.id", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("tittle","content");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","tittle","content");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'tittle' => 'required',
	...
	function delete($rec_id = null){
		...
		$db->where("document.id", $arr_rec_id, "in");
